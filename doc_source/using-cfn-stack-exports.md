...
To export a stack's output value, use the `Export` field in the Output section of the stack's template\. To import those values, use the `Fn::ImportValue` function in the template for the other stacks\. For a walkthrough and sample templates, see [Walkthrough: Refer to Resource Outputs in Another AWS CloudFormation Stack](walkthrough-crossstackref.md)\.
