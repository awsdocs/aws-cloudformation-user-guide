...
Changing this value triggers an update of the node group if one is available\. You can't update `Labels`, `ScalingConfig`, `Taints`, or `UpdateConfig` at the same time as updating `Release Version` or `Version`\. Doing so results in the following error: `Version and ReleaseVersion updates can't be combined with other updates.`
